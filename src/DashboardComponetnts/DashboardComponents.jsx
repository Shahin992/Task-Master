...
const DashboardComponents = () => {
  ...
  const addItemToSection = (id, newStatus) => {
    axios
      .put(`https://task-management-server-main-ruby.vercel.app/tasks/${id}`, {
        status: newStatus,
      })
      .then((data) => {
        refetch();
    ...
    <div>
      ...
      <div className="flex flex-col lg:flex-row gap-5 justify-between p-2">
        <div ref={dropToDo} className="flex-1 min-h-0 lg:min-h-screen">
          <h1 className="text-center py-3 bg-[#008844] rounded-2xl mb-5 text-sm lg:text-xl font-semibold lg:font-bold text-white">
            To Do{" "}
            <span className="px-1 w-8 h-8 ml-3 rounded-full bg-white text-black">
              {fToDo.length}
            </span>
          </h1>
          <div>
        ...
        <div ref={dropOnGoing} className="flex-1 min-h-0 lg:min-h-screen">
          <h1 className="text-center py-3 bg-[#12e71d] rounded-2xl mb-5 text-sm lg:text-xl  font-semibold lg:font-bold text-white">
            On Going{" "}
            <span className="px-1 w-8 h-8 ml-3 rounded-full bg-white text-black">
              {fOnGoing.length}
            </span>
        ...
        <div ref={dropCompleted} className="flex-1 min-h-0 lg:min-h-screen">
          <h1 className="text-center py-3 bg-[#12e71dc4] rounded-2xl mb-5 text-sm lg:text-xl  font-semibold lg:font-bold text-white">
            Completed{" "}
            <span className="px-1 w-8 h-8 ml-3 rounded-full bg-white text-black">
              {fComplete.length}
            </span>
